...
ISR( INT6_vect ) {
  ...
  //byte state = 0;
  e1_state = e1_state >> 2;
  e1_state = e1_state | ( newE1_A  << 3 );
  e1_state = e1_state | ( newE1_B  << 2 );
  
  // Register change of direction
  if( e1_state == 1 ) {
    e1_count--;
  } else if( e1_state == 7 ) {
    e1_count--;
...
ISR( PCINT0_vect ) {
  ...
  //byte state = 0;
  e0_state = e0_state >> 2;                   
  e0_state = e0_state | ( newE0_A  << 3 );
  e0_state = e0_state | ( newE0_B  << 2 );
